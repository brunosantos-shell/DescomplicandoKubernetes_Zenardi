...
- [Overview](#overview)
  - [Usos para Secrets](#usos-para-secrets)
    - [Alternativas a Secrets](#alternativas-a-secrets)
  - [Trabalhando com Secrets](#trabalhando-com-secrets)
    - [Criando um Secret](#criando-um-secret)
      - [Restrições de nomes de Secret e dados {#restriction-names-data}](#restrições-de-nomes-de-secret-e-dados-restriction-names-data)
      - [Limite de tamanho {#restriction-data-size}](#limite-de-tamanho-restriction-data-size)
    - [Editando um Secret](#editando-um-secret)
    - [Utilizando Secrets](#utilizando-secrets)
      - [Secrets Opcionais {#restriction-secret-must-exist}](#secrets-opcionais-restriction-secret-must-exist)
    - [Utilizando Secrets como arquivos em um Pod {#using-secrets-as-files-from-a-pod}](#utilizando-secrets-como-arquivos-em-um-pod-using-secrets-as-files-from-a-pod)
      - [Projeção de chaves de Secrets em caminhos específicos](#projeção-de-chaves-de-secrets-em-caminhos-específicos)
      - [Permissões de arquivos de Secret](#permissões-de-arquivos-de-secret)
      - [Consumindo valores de Secrets em volumes](#consumindo-valores-de-secrets-em-volumes)
      - [Secrets montados são atualizados automaticamente](#secrets-montados-são-atualizados-automaticamente)
    - [Utilizando Secrets como variáveis de ambiente {#using-secrets-as-environment-variables}](#utilizando-secrets-como-variáveis-de-ambiente-using-secrets-as-environment-variables)
      - [Variáveis de ambiente inválidas {#restriction-env-from-invalid}](#variáveis-de-ambiente-inválidas-restriction-env-from-invalid)
      - [Consumindo valores de Secret em variáveis de ambiente](#consumindo-valores-de-secret-em-variáveis-de-ambiente)
    - [Secrets para obtenção de imagens de contêiner {#using-imagepullsecrets}](#secrets-para-obtenção-de-imagens-de-contêiner-using-imagepullsecrets)
      - [Usando `imagePullSecrets`](#usando-imagepullsecrets)
        - [Especificando `imagePullSecrets` manualmente](#especificando-imagepullsecrets-manualmente)
        - [Configurando `imagePullSecrets` para serem adicionados automaticamente](#configurando-imagepullsecrets-para-serem-adicionados-automaticamente)
    - [Utilizando Secrets com pods estáticos {#restriction-static-pod}](#utilizando-secrets-com-pods-estáticos-restriction-static-pod)
  - [Casos de uso](#casos-de-uso)
    - [Caso de uso: Como variáveis de ambiente em um contêiner](#caso-de-uso-como-variáveis-de-ambiente-em-um-contêiner)
    - [Caso de uso: Pod com chaves SSH](#caso-de-uso-pod-com-chaves-ssh)
    - [Caso de uso: Pods com credenciais de ambientes de produção ou testes](#caso-de-uso-pods-com-credenciais-de-ambientes-de-produção-ou-testes)
    - [Caso de uso: _dotfiles_ em um volume de Secret](#caso-de-uso-dotfiles-em-um-volume-de-secret)
    - [Caso de uso: Secret visível somente em um dos contêineres de um pod {#use-case-secret-visible-to-one-container-in-a-pod}](#caso-de-uso-secret-visível-somente-em-um-dos-contêineres-de-um-pod-use-case-secret-visible-to-one-container-in-a-pod)
  - [Tipos de Secrets {#secret-types}](#tipos-de-secrets-secret-types)
    - [Secrets de token de service account (conta de serviço)](#secrets-de-token-de-service-account-conta-de-serviço)
    - [Secrets de configuração do Docker](#secrets-de-configuração-do-docker)
    - [Secret de autenticação básica](#secret-de-autenticação-básica)
    - [Secret de autenticação SSH](#secret-de-autenticação-ssh)
    - [Secrets TLS](#secrets-tls)
    - [Secret de token de autoinicialização {#bootstrap-token-secrets}](#secret-de-token-de-autoinicialização-bootstrap-token-secrets)
  - [Secrets imutáveis {#secret-immutable}](#secrets-imutáveis-secret-immutable)
    - [Marcando um Secret como imutável {#secret-immutable-create}](#marcando-um-secret-como-imutável-secret-immutable-create)
  - [Informações de segurança sobre Secrets {#information-security-for-secrets}](#informações-de-segurança-sobre-secrets-information-security-for-secrets)
    - [Recomendações de segurança para desenvolvedores](#recomendações-de-segurança-para-desenvolvedores)
    - [Recomendações de segurança para administradores de cluster](#recomendações-de-segurança-para-administradores-de-cluster)
...
2. Habilite ou configure [regras de RBAC](/docs/reference/access-authn-authz/authorization/)
